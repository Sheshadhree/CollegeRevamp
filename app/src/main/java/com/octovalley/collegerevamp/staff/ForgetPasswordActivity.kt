...
class ForgetPasswordActivity : AppCompatActivity() {
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    if(snapshot.exists())
                    {
                        if(snapshot.child("Security Questions").exists())
                        {
                            ...
                            if(ans1==answer1&&ans2==answer2)
                            {
                                ...
                                builder.setPositiveButton("Change"){dialog , which->
                                    ref.child("password").setValue(newPassword.text.toString()).addOnCompleteListener{task->
                                        ...
                                        {
                                            Toast.makeText(this@ForgetPasswordActivity,"Password changed Successfully...!",Toast.LENGTH_LONG).show()
                            ...
                            {
                                Toast.makeText(this@ForgetPasswordActivity,"Answers are wrong...!",Toast.LENGTH_LONG).show()
                            }
                        }
                        else
                        {
                            Toast.makeText(this@ForgetPasswordActivity,"You have not answered Security Questions...!",Toast.LENGTH_LONG).show()
                        }
                    }
                    else
                    {
    ...
    private fun displayPreviousAnswers() {
        val ref = FirebaseDatabase.getInstance().reference
                .child(Staffvalent.currentOnlineStaff?.collegeCode.toString())
                .child(Staffvalent.currentOnlineStaff?.phoneNumber.toString())
